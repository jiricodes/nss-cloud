...
<a id="figure-1">![Figure 1](pictures/fig-1-nextcloud-setup.png)</a>
...
Bandwidth should not be a large issue for our system as a personal cloud should not be seeing very much usage from multiple concurrent users, but unnecessary data loading is avoided as a rule so that the user experience can remain smooth.
...
As a personal cloud, most users will likely only have one administrator (themselves), but if they so desire, Nextcloud supports designating multiple administrators, and further-specified roles can be delegated to support customized administration topology. [\[NC-WP\]](#nc-wp)
...
For personal cloud, this is largely irrelevant. The infrastructure of personal home server wouldn't normally be that big, and when renting cloud services, a personal server doesn't need unpredictable, scalable solutions. In the data transfer itself, in Finland, the only thing that usually costs something is the bandwidth and not the amount of data (at least on personal scale). For users that have limited data, this can be avoided by avoiding loading unnecessary data or compressing file transfers. For example, only small previews of images are downloaded automatically to save data.
...
### 7.2 Federation
People on different servers can share files together. Though people still need to log into their own server, this can help to extend the system and make communication between users of different servers possible.
...
However, if users' needs to exceed the supply of native applications Nextcloud's WebDAV standard compliant API gives an opportunity to independently create custom services.
...
To get a more holistic understanding of how NSSCloud performs under load we would need to do a more comprehensive benchmarking suite. We would need to profile and benchmark the NSSCloud code with various different types of request. For example our AB test only requested the same welcome page over and over again. It is likely that after receiving the first request, rest of the received requests were served from cache. In a broader we should create tests were the service is forced to dynamically create the response content for every request. This would give more insight into how the server copes under differing loads as well as mitigate the role of cache on the test results.
...
Nextcloud feels like a great DIY cloud playground, with many potential areas for customization and extension depending on one's particular use case. By the end of the project, we were able to fully implement the project as originally defined (and extended by our goals) in the topics: a personal file-serving cloud using Nextcloud, with users authorized using LDAP and connected via HTTPS, using Let's Encrypt certificates. Perhaps the most challenging part of the project was setting up LDAP, which did not in fact become fully functional until just a day or two before the due date. At first, the easiest method seemed to be just to use the default LDAP installation on the Ubuntu VM that the project was running in. After several hours of work, it became clear that it was not the greatest, so a more custom solution was sought. Eventually, OpenLDAP was settled on, though this came with its own issues: the LDAP server was unable to verify certificates from the clients. After a lot more work, the solution ended up being to simply disable server verification of certificates. Since the client was still able to verify the server's certificate, this meant that a secure connection could still be established, as two-way verification is not strictly neccessary for a secure TLS connection.

One of the areas where we ran into problems was with trying to setup the database for the Nextcloud server. Originally, we started out with MariaDB, which is recommended for use with Nextcloud according to their documentation. But, we started to run into problems when it came to the Nextcloud server connecting to the database, and got a strange error message: "Error while trying to initialise the database: An exception occurred while executing a query: SQLSTATE[HY000]: General error: 4047 InnoDB refuses to write tables with ROW_FORMAT=COMPRESSED or KEY_BLOCK_SIZE." At first, we tried to debug it, but after many unsuccesful Google searches and perusals of the Nextcloud documentation and support material, we decided to pivot. While MariaDB and mySQL are recommended for usage with Nextcloud, according to the documentation PostgreSQL is supported as well. So, we decided that our best option was to try and replace MariaDB with PostgreSQL. Despite not being recommended by Nextcloud, PostgreSQL ended up working for us better than MariaDB, and so our system ended up using PostgreSQL. The takeaway for us is that when it comes to open source software and modules, there are often many comparable options, and if one doesn't work, then another one might work fine without losing any essential functionality. This was a commmon theme both in our LDAP and our database problem-solving.
